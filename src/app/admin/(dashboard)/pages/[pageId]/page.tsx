...
export default function PageEditor({ params }: { params: Promise<{ pageId: string }> }) {
    ...
    const pageLabels: Record<string, string> = {
        ...
        photography: 'Photography', videography: 'Videography', 'digital-marketing': 'Digital Marketing',
    };
    ...
    return (
        <div className="max-w-5xl mx-auto pb-24">
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-8">
                ...
                {pageId === 'photography' && <CreativeServicePageEditor pageId="photography" get={get} set={set} />}
                {pageId === 'videography' && <CreativeServicePageEditor pageId="videography" get={get} set={set} />}
                {pageId === 'digital-marketing' && <CreativeServicePageEditor pageId="digital-marketing" get={get} set={set} />}
...
function BookingFields({ get, set }: { data: Record<string, unknown>; get: (k: string, fb?: unknown) => unknown; set: (k: string, v: unknown) => void }) {
    ...
    const addPkg = () => set('packages', [...packages, { name: 'New Package', description: '', price: 'From RWF 0' }]);
                ...
                <div className="flex flex-col gap-6">
                    {packages.map((pkg, i) => (
                        <div key={i} className="bg-[#1a1812] p-6 rounded-2xl border border-white/5 flex flex-col gap-4 hover:border-[#ffc000]/30 transition-colors shadow-sm">
                            ...
                            <TextInput label="Price Label" value={pkg.price} onChange={v => updatePkg(i, 'price', v)} placeholder="From RWF 2,500,000" />
                        </div>
                    ))}
